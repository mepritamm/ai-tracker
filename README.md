...
- **Board** — the home view: the session rail down the side, a triage strip, and a board of tiles that's never more than 8 at once (pinned sessions first, then by state and recency — everything past the cap lives in the rail, not on the board). Each tile shows the session's name and its working directory, a short "what it's doing now" line underneath, a done/total todo count, and — when the tool has recorded one — its **model** (e.g. `sonnet 4.5`), tacked onto the same trailing "age · tool" line. A tile whose most recently completed command errored shows a **failing** state instead — `fail: <command>` — for Claude and Auggie sessions; the Augment extension has no readable command transcript to derive that signal from, so its sessions never show as failing. Rail rows carry the same directory + model detail, plus a **group-by** control: directory · activeness · last 24h · 7 days · 30 days · none.
...
- **Mobile / tablet.** The top bar wraps at narrow widths, every destination stays reachable, touch targets are 44px at phone width, and the rail becomes an overlay drawer below 1024px. Session detail gets its own phone layout: a back chevron + ellipsed breadcrumb status bar, a 34px presence orb with the live narration set in serif type just below it, and — when the session is waiting on you — an awaiting-question card ahead of the folded State and Evidence columns.
