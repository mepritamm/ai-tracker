...
- **Resuming a running background agent** — Claude Code refuses to `--resume` a session that's still running as a background agent. The tracker detects that case and adds `--fork-session`, so you get a branch off it rather than a refusal. Note that's a *copy*, not the live agent; `claude agents` is the way to attach to the original.
