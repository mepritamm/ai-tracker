...
**Prerequisites:** **Python 3.8+**. That's it — the Python side uses only the standard library, so there is **nothing to `pip install`** and no Python build step. The terminal renders with a vendored copy of xterm.js by default (fetched only when you open a terminal, never on page load); switch to the stdlib VT100 emulator instead with `TRACKER_TERM_RENDERER=grid`, or per terminal from the toolbar.
...
- **How many terminals at once** — **12** by default (`TRACKER_MAX_TERMS`, clamped to 1–64). A cap exists because every live terminal pins a real child process, a reader thread and a screen buffer — but watching several sessions at once is the point of this app, so it is set well above what you're likely to need and it is not a dead end when you do hit it. Closing the dialog deliberately *detaches* rather than kills (that's what lets **⤢ New tab** reopen the same shell), so hitting the cap tells you exactly which terminals are holding the slots — command, directory and age — with a **✕** on each that kills it and immediately opens the one you asked for. That same list is available on demand too, not only when you hit the cap: the sidebar's third button, **☰ Manage terminals**, opens a panel headed `N of <max> running` listing every live terminal the same way — command, directory, age — with a **peek** (opens that terminal in its own tab, carrying its session so the model/effort switchers come along) and a **✕ kill** on each row, plus a **Close all** in the footer behind an inline confirmation (it kills every running terminal, including any Claude session inside one, and cannot be undone). Works from a phone or tablet like everything else here.
...
- **Resuming a running background agent** — Claude Code sometimes refuses to `--resume` a session that is running as a background agent, and `--fork-session` is the only programmatic way past it. Rather than trying to predict that (three different signals — the transcript's `sessionKind`, the `claude agents` registry, and a live pid — all failed to), the tracker just **reacts**: if a resume comes back with that refusal, it retries once with `--fork-session`, and the external-terminal path carries the same fallback in its shell command. In the browser terminal the whole recovery is **invisible** — the pane shows `starting…` and holds its stream open while the refused child exits and the replacement starts, so you never see the refusal or a reconnect, just the terminal coming up. (It is not instant: measured, the refusal prints ~2s in and the child exits ~0.6s after that.) Note what a fork gives you: a **copy**, under a new session id, with the original agent still running. There is no non-interactive way to attach to the live one — `claude agents` is an interactive picker with no attach verb and no id argument. The tracker follows the copy: there is no fork lineage recorded on disk anywhere (a transcript's `parentUuid` only points to the previous message in the *same* session, not to a parent session), so the instant it forks is the only moment the link is knowable, and that's when the tracker records it: it snapshots which transcripts already exist just *before* the fork execs, and then claims the new one that also carries the parent's copied message `uuid`s. Both conditions are exact, which matters more than it sounds — earlier attempts identified the copy by directory and timestamp, and each one could be made to point confidently at an unrelated session. If neither condition is met (the copy never appears, or the snapshot couldn't be taken), the tracker leaves the session unlinked rather than guessing. Both the sidebar and the detail page carry it afterward — the parent's detail page shows a banner pointing at the live copy, and the copy carries a quieter back-link to where it came from. See [`docs/claude-resume-command-matrix.md`](docs/claude-resume-command-matrix.md) for the tested behaviour of every session state.
