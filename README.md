...
- **A persistent top bar with three destinations — Board · Sessions · Terminals** — plus the session rail, stay on screen across views; neither used to survive opening a session. Inside an open session the rail collapses to a **56px orb rail** by default, letting you switch to another session with one click; the toggle button expands it back to full width on demand. Collapsed, its orbs stay centered — the scrollbar track that used to nudge them sideways as sessions came and went is hidden outright, not just narrowed — and its footer swaps the expanded rail’s **Show N more** control for a compact **+N** (the full sentence still rides the tooltip, since the phrase itself doesn't fit a 48px column). **Which session is selected is shared with the classic dashboard** — pick one in either UI and the other is on that same session, because both read and write the one selection store (the control room used to keep a private copy that started empty, so a session picked in the classic sidebar highlighted nothing here). The one place the rail *doesn't* show is the Sessions destination's own browse list (below) — it has no rail of its own since it already **is** the session browser. Its own rail toggle, flag count and notification bell carry visible words too — **Sessions**, **Flags N**, **Alerts** — next to their icons, the same treatment Config/Help/New session already had. **The top bar stays one 54px row from 1280px up** — the tier just below full-width compacts its text-and-icon controls to icon-only (title/`aria-label` still carry the accessible name) instead of wrapping to a second row.
- **Board** — the home view: the session rail down the side, a **triage strip**, and a board of tiles capped at **3–12 at once** (default 8, adjustable in the Config dialog's Board tab; pinned sessions first, then by state and recency — everything past the cap lives in the rail, not on the board). The triage strip has **four** clickable/filterable cells — **Waiting on you · Working · Flagged · Pinned** — each a large serif count over a small uppercase label, styled to the design docs; clicking one filters the board/rail to just that state, clicking it again clears the filter. **Working** counts a session whose own foreground turn has already ended but whose background agents are still running, not just one still mid-turn — the classic sidebar's own live/done read agrees with it, since both now derive from the same shared predicate rather than two separately-maintained copies. Every tile carries a "project · tool" sub-line under its title, shows the session's working directory, a short "what it's doing now" line, a done/total todo count, and — when the tool has recorded one — its **model** (e.g. `sonnet 4.5`), tacked onto the same trailing "age · tool" line. Every **Working**-state tile also gets an agent-coloured border and soft glow, not only tiles with a live background agent. A tile whose most recently completed command errored shows a **failing** state instead — `fail: <command>` — for Claude and Auggie sessions; the Augment extension has no readable command transcript to derive that signal from, so its sessions never show as failing. Rail rows carry the same directory + model detail, a **group-by** control (directory · activeness · last 24h · 7 days · 30 days · none), and now the same session-management controls as the classic sidebar: a clickable **"N live"** filter, **pin/unpin**, **rename**, and the same re-run collapse as the board (repeated runs of one background agent fold into one row tagged **×N**). Rail rows are now at **full information parity with the classic sidebar**, and the rail is the same **300px** width as it: each row also carries the **project name**, the **source label** (cli · desktop · VS Code · Auggie) and an **answer / done** status badge (drawn from the shared icon sprite, so it follows the icon-style setting), and shows its flag count, note count, background-agent count and age *together* — previously the first of those to be present hid the rest, so a flagged row lost its age entirely. The row is **stacked like classic’s own** — a title line over a wrapping meta line — rather than one horizontal strip: the metadata used to be an unshrinkable flex item, so on a 300px rail it squeezed the session title down to a character or two. The flag and note counts are **real badges on the title line, and clickable**: the flag badge opens the Flags dialog, the note badge and the background-agent chip open the session (a badge click never also opens the row behind it). A parent row also carries classic’s **agent-children chip** (*2 live / 5 agents*), the one field of classic’s row the rail previously had no equivalent for. **The rail shows the newest 25 unpinned sessions**, with a **Show N more** footer that reveals the next 25 — pinned sessions are pinned precisely so they stay visible and never fall off that cap. Each row also picks up the same row-level state tinting classic uses for pinned/waiting/done/flagged (the whole row, not just the dot or a buried status word) and a **hasagents** marker — the same amber-bordered treatment classic gives a session that has spawned separate agent sessions of its own. A **done** session takes a green fill as well as its green edge; a session that is *both* pinned and done keeps its amber pinned fill, matching classic, where `.sitem.done` sets no background at all.
