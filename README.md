...
- **Help and Config are dialogs, not routes.** Config's own preferences — theme, session rail, cards-start-folded, poll interval, notification sound — are live browser settings that save as you change them. The real running parameters (LIVE_WINDOW, TRACKER_TERM_RENDERER, TRACKER_MAX_TERMS, TRACKER_TERMINAL, TRACKER_AUTH — shown only as set/not-set, TRACKER_TERM_APP, TRACKER_TERM_ALLOW, PORT, HOST) are shown alongside them but are **read-only**: there's no route to change them from the browser, so env vars stay the actual source of truth and each row states its restart cost. Board tiles stay capped at 8 by design, so that row is read-only too.
