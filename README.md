...
**Main view** for the selected session — the header carries a progress ring, title, and stat chips; below it the body splits into two halves, **◐ State** (progress, plan, decisions, summary) and **◑ Activity** (narration, prompts, files, PRs, commands), collapsing to one column on narrow screens. A **☀️/🌙 theme toggle** next to the 🔔 bell switches between the default dark and a warm-beige light mode (remembered across visits); background agents & shells live in a right-side drawer.
