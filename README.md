...
- **How many terminals at once** — **12** by default (`TRACKER_MAX_TERMS`, clamped to 1–64). A cap exists because every live terminal pins a real child process, a reader thread and a screen buffer — but watching several sessions at once is the point of this app, so it is set well above what you're likely to need and it is not a dead end when you do hit it. Closing the dialog deliberately *detaches* rather than kills (that's what lets the **New tab** button reopen the same shell), so hitting the cap tells you exactly which terminals are holding the slots — each row names the session running in it (by title, falling back to project or a short id) instead of the identical `claude --resume <uuid>` every such row used to show, plus its project (the cwd's trailing segment, not the shared leading one) and age, with the full command and directory a hover away — with a **kill** button on each that closes it and immediately opens the one you asked for. That same list is available on demand too, not only when you hit the cap: the sidebar's **Manage terminals** button — badged with the live running count when the feature's reachable (no badge otherwise), read off the sidebar's own poll rather than a fetch of its own — opens a panel headed `N of <max> running` listing every live terminal the same way, with a **peek** action (opens that terminal in its own tab, carrying its session so the model/effort switchers come along) and a **kill** action on each row, plus a **Close all** in the footer behind an inline confirmation (it kills every running terminal, including any Claude session inside one, and cannot be undone). Works from a phone or tablet like everything else here.
...
- A session running **in-transcript background agents** (Task/Workflow subagents — which spawn no separate session) carries an **N running** badge, so you can see it's busy without opening it.
- **End-state at a glance** — a session **waiting on your answer** (an unanswered `AskUserQuestion` / Auggie `ask-user`) is flagged **answer** with an amber highlight so you know to go respond; one that **just completed its last run** (the last turn was the assistant finishing, within the live window) shows a subtle **done** indicator — gated to fresh completions so it flags what just landed rather than every stale session. Both work for every tool.
- **Click "N live"** to filter to only active sessions (live = touched in the last 5 minutes).
- **Search** by keyword — all three search boxes (classic dashboard, session rail, Sessions view) share one query and one full-transcript result set, so a phrase found deep in a conversation shows everywhere. Sessions whose *name* matches rank first.
- **Rename** any session to a title that means something to you (saved to `titles.json`).
- **Pin** any session to keep it at the top of the list, above recency (saved to `pins.json`); the pinned marker is now the same blue in both classic and Control Room views.
- Sessions with todos show a **todo badge** — a compact `done/total` count, the same `todo_done`/`todo_total` the Control Room's rail rows and board tiles already drew, so progress reads the same in both UIs. Claude and Auggie alike; sessions with no todos show nothing rather than `0/0`.
- Sessions with notes show a **notes** badge (see the Plan-on-the-go panel below).
- Sessions with open flags show a **flags** badge and a red edge, and the sidebar's own **Flags** button (next to the notification bell) opens **every session's flags in one list** — each row names the session it belongs to and clicks through to it. Without it, a flag raised on a session you aren't currently looking at is invisible. Resolve / reopen / delete work from that list too, on any screen size.

**Main view** for the selected session — the header carries a progress ring, title, stat chips, a **search-this-session** toggle, and a **Flag an issue** button; below it the body splits into two halves, **◐ State** (progress, PRs, plan, decisions, summary) and **◑ Activity** (narration, prompts, files, commands), collapsing to one column on narrow screens. Every panel collapses to just its header — click the title (or the **▾/▸** chevron) to fold a panel away and focus on what you care about. A **theme toggle** next to the notification bell switches between the default dark and a warm-beige light mode (remembered across visits); background agents & shells live in a right-side drawer. The Flags panel is opt-in — the header toggle reveals it above the State/Activity split, and it opens by itself when the session you're viewing has an open flag.
- **Search within the open session** — the header search button reveals a full-width search card above the State/Activity split (like the Flags panel). It matches across this session's narration, prompts, files, commands and todos in one place; each hit shows a highlighted snippet and clicks through to the same pop-out modal (diff for files, output for commands, full text otherwise). Works for every tool.
- **Waiting, not idle** — when a session stopped on an unanswered question, the header reads **waiting on you · &lt;age&gt;** (amber) instead of "idle", and the banner says *Waiting on your answer*. Same signal as the sidebar's **answer** status, so a blocked session never reads as merely stale. Works for every tool.
...
- **Links** — the URLs and local file paths the session generated or worked on, gathered from its PRs, files, narration, prompts and commands into one panel: a PR it opened or a file it wrote is tagged **generated** (with the actual verb — created / wrote / merged / closed); a URL it merely cited elsewhere is tagged **worked on**. An http(s) link is clickable; a local path renders as plain, inert text. A local path that no longer exists on disk is quietly left out of this panel — nothing is deleted from the session's own history (the Files panel below still lists it, and `counts.created`/`counts.edited` still report the session's true totals) — while a **remote** URL is never checked, dead or not, because the tracker makes no outbound requests, ever. Shared with the control room below — one `deriveLinks()` function, not two.
- **Narration** — the assistant's own words, step by step, with full markdown rendering (tables, code, lists) in the pop-out modal — each code block has its own one-click **Copy** button, and a ` ```mermaid ` fence is drawn as an actual **diagram** instead of raw source. Diagrams render for real, via a vendored, lazily-loaded copy of **mermaid.js** (v11.17.2, MIT — `aitracker/web/vendor/mermaid.min.js`; fetched only when a diagram is actually about to render, the same pattern the terminal's xterm.js uses) — so any diagram family mermaid.js itself understands is covered, not just a fixed list. Before that asset loads — or if it fails to (offline, blocked, or a source it can't parse) — a hand-rolled SVG renderer draws the diagram instantly and locally, no library involved, covering the families that actually turn up in agent-generated markdown: **flowcharts** (`flowchart`/`graph` — node shapes, edge labels, `classDef` colours), **sequence diagrams** (`sequenceDiagram` — participants, messages, self-loops, `Note over`/`left of`/`right of`, `alt`/`opt`/`loop`/`par` blocks with nesting), **state diagrams** (`stateDiagram`/`stateDiagram-v2` — states, `[*]` pseudostates, labelled transitions, aliased states, composite states flattened), **class diagrams** (`classDiagram` — classes with members, every relationship kind, cardinality on the edge), **ER diagrams** (`erDiagram` — entities with attributes, cardinality translated to readable prose), **user journeys** (`journey`/`userJourney` — sections with tasks and happiness-score faces), **pie charts** (`pie` — labelled slices with legend and percentages), and **quadrant charts** (`quadrantChart` — 2×2 axes with plotted points). Any other diagram type (`gantt`, `mindmap`, `timeline`, `gitGraph`, `xychart-beta`, …) shows as tagged code — so the intent is visible rather than mistaken for a plain code fence — until mermaid.js is available, then upgrades to a real render like everything else. This holds anywhere markdown is rendered: narration, prompts, todos, notes, the summary's Goal/Now/So-far fields, decisions & open questions, the agents panel's task text, background-agent/shell output and the Rendered-markdown view of a `.md` file — except shell commands, file paths, and terminal/PTY output, kept deliberately literal because `*`/`_` are meaningful characters there. On the machine-output surfaces (a background shell's stdout, a background agent's last message) a restricted renderer keeps bold, inline code and links but drops single-asterisk italics, so a glob like `*.py` or a `*** 3 failed ***` separator line survives intact instead of being read as emphasis. Plus prev/next arrows and a jump-to-latest button across every entry. History is unbounded — older entries page in from the server as you scroll. An open entry stays live: it follows the newest message, or holds your place if you've paged back into history.
...
A second UI sits alongside the dashboard above (which is unchanged apart from one entry point). Open it from the **Try the new experience** button in the classic header — it now sits right next to **Flag an issue** in the header's action pair — or by adding `?ui=next` to the URL (`?ui=classic` switches back); either way there's one click back to classic, any time, no reload. The choice persists in `localStorage` (`tracker.ui` = `classic` | `next`). It reads the same server data over the same shared seam — no server change, no build step, no new dependency. **One shared UI state across both views** — the selected session, search query, live-only filter, active Board tab, "Show all" toggle state, expanded agent groups, and scroll position are synchronized between classic and Control Room, and carry across browser tabs.

- **A persistent top bar with three destinations — Board · Sessions · Terminals** — plus the session rail, stay on screen across views; neither used to survive opening a session. Inside an open session the rail collapses to a **56px orb rail** by default, letting you switch to another session with one click; the toggle button expands it back to full width on demand. Collapsed, its orbs stay centered — the scrollbar track that used to nudge them sideways as sessions came and went is hidden outright, not just narrowed — and its footer swaps the expanded rail's **Show N more** control for a compact **+N** (the full sentence still rides the tooltip, since the phrase itself doesn't fit a 48px column). The one place the rail *doesn't* show is the Sessions destination's own browse list (below) — it has no rail of its own since it already **is** the session browser. Its own rail toggle, flag count and notification bell carry visible words too — **Sessions**, **Flags N**, **Alerts** — next to their icons, the same treatment Config/Help/New session already had. **The top bar stays one 54px row from 1280px up** — the tier just below full-width compacts its text-and-icon controls to icon-only (title/`aria-label` still carry the accessible name) instead of wrapping to a second row. **Alerts shows a struck-through bell when notifications are muted**, reading the same single setting classic’s own bell writes (one mute state for the whole app, not a second switch), with the title and aria-label changing too so the state never rides on the icon alone. The destination controls size consistently and their trailing counts sit in a fixed-width slot, so a long count can no longer push the row wide enough to clip the controls at its right edge — the labelled tier now applies only at widths where it genuinely fits, and compacts to icon-only below that.
- **Board** — the home view: the session rail down the side, a **triage strip**, and a board of tiles capped at **3–12 at once** (default 8, adjustable in the Config dialog's Board tab). The board never reads as empty — while sessions exist, it shows a default mix ranked by attention (waiting on you, working, flagged first), then by most recent activity — pinned is a marker and a triage filter, not a board sort key — backfilled with most recently *modified* idle sessions so the board always has content. The triage strip has **four** clickable/filterable cells — **Waiting on you · Working · Flagged · Pinned** — each showing a count and label; clicking one filters the board to just that state, and clicking it again clears the filter. The filter applies first, then the tile limit, so a cell's count always matches the board content below it. **Working** counts a session whose own foreground turn has already ended but whose background agents are still running, not just one still mid-turn — the classic sidebar's own live/done read agrees with it, since both now derive from the same shared predicate rather than two separately-maintained copies. A **"Show all"** toggle in the board's cap footer lifts the tile limit for the current tab (it resets when you switch tabs). The footer's count shows the current view ("5 of 40") rather than always showing every session. Every tile carries a "project · tool" sub-line under its title, shows the session's working directory, a short "what it's doing now" line, a done/total todo count, and — when the tool has recorded one — its **model** (e.g. `sonnet 4.5`), tacked onto the same trailing "age · tool" line. Tiles are coloured by state: **Waiting on you** tiles render orange-amber, **working** tiles (and every working-state row in the rail) also get an agent-coloured border and soft glow (not only tiles with a live background agent), **pinned** tiles carry a blue left accent, and **failing** tiles appear red. A tile whose most recently completed command errored shows a **failing** state instead — `fail: <command>` — for Claude and Auggie sessions; the Augment extension has no readable command transcript to derive that signal from, so its sessions never show as failing. Rail rows carry the same directory + model detail, a **group-by** control (directory · activeness · last 24h · 7 days · 30 days · none), and now the same session-management controls as the classic sidebar: a clickable **"N live"** filter, **pin/unpin**, **rename**, and the same re-run collapse as the board (repeated runs of one background agent fold into one row tagged **×N**). Rail rows are now at **full information parity with the classic sidebar**, and the rail is the same **300px** width as it: each row also carries the **project name**, the **source label** (cli · desktop · VS Code · Auggie) and an **answer / done** status badge (drawn from the shared icon sprite, so it follows the icon-style setting), and shows its flag count, note count, background-agent count and age *together* — previously the first of those to be present hid the rest, so a flagged row lost its age entirely. The rail shows the newest sessions with a **Show N more** footer that reveals the next page — pinned sessions are pinned precisely so they stay visible and never fall off that cap, and the count beside it now counts only rows the button can actually reveal, so it reaches zero. A **done** session takes a green fill as well as its green edge; a session that is *both* pinned and done keeps its pinned fill, matching classic, where `.sitem.done` sets no background at all. The rail no longer loses background-agent sessions — a plain `claude --bg` agent (one not part of a named agent group) used to be invisible in the rail, in the collapsed icon rail and in the Sessions count, while the board and the classic sidebar both listed it. Each row also picks up the same row-level state tinting classic uses for pinned/waiting/done/flagged (the whole row, not just the dot or a buried status word) and a **hasagents** marker — the same amber-bordered treatment classic gives a session that has spawned separate agent sessions of its own. The rail now survives a search from the classic dashboard and correctly shows when no results match (instead of disappearing). A session that is failing now shows as failing in the rail instead of idle, and keyboard focus on a rail row survives the 2-second refresh. Scrolling to the very top of the rail now sticks, and the collapsed icon rail shows a clearable indicator when a search or live-only filter is hiding sessions. The "+N more" footer counts only what is actually hidden from the current view. **Session-state colours are now one palette across both UIs** — waiting-on-you, working, landed, flagged and failing resolve to the same value in the classic dashboard and the control room, per theme, instead of each view carrying its own near-miss shade. **Working (live) reads green** and carries an **always-on glow** — on the rail row’s dot, on the collapsed orb, on the board tile, and in the session’s own header once opened — so a session that is actually running is identifiable at a glance from any view. Under `prefers-reduced-motion` the pulse stops but the glow stays, so the signal never depends on motion; classic honours that setting now too, which it previously ignored entirely.
- **Sessions** — each row carries the **same** segmented todo bar and "N background agents" line the board tiles draw (one shared `todoTicks()`, not a second copy), so moving between the two destinations no longer changes how far along a session looks. A top-bar destination of its own: opens the last-opened (or most-recently-active) session with the rail present, or — when there's none — a browse list with a search box and **pagination** (10 / 25 / 50 per page). Its search goes through `/api/search`, the same server-side search the classic sidebar uses, rather than filtering the already-fetched session list client-side — `/api/list` caps at 200 sessions per provider, so a client-side filter would silently miss older ones. Its box, the rail's and the classic sidebar's are now **one** search — they share a single query and a single result set, so a phrase found deep in a transcript surfaces in all three rather than only the one that asked the server.
...
- **Session detail** — full-width, three columns of panels, every one collapsed by default except the conversation timeline, which merges narration and prompts into a single chat-style thread (newest entries at the top) instead of two separate panels. The header's title is the session's **name** (matching the classic dashboard), with the last prompt demoted to its own goal line below rather than standing in for the title. Its header actions — **Search**, **Flag**, **Rename** and (on phone) **Stop** — carry visible text labels next to their icons too, same as the classic dashboard's own buttons; they compact to icon-only at ≤900px, and the header's wider action buttons compact in the narrower 901–1300px band just above that. The TIMELINE panel header carries four individually-selectable filter chips — **prompts**, **narration**, **tools**, **results** — alongside the existing all/talk-only presets, plus a button to pop out the newest entry in the full-screen modal (or click any entry to open it); the same **openText()/_setNav()** modal the classic dashboard's narration pop-out uses. Timeline entries and notes render through the same markdown path as the classic dashboard's narration — every fenced code block gets its own **Copy** button, and a ` ```mermaid ` fence upgrades to a real diagram, both reusing app.js's existing `mdBlock()`/`copyCode()` rather than a second implementation. If the session's transcript has a line that failed to parse, the timeline says so — *"Couldn't read this session — the transcript exists but a line failed to parse. Everything before it is shown"* — instead of silently rendering a partial transcript as if it were complete (see the parity table above for the Claude/Auggie difference). Shares the classic dashboard's **Links** panel described above (the same `deriveLinks()`) — though only here does the Files panel dim a dead local path in place, titled "No longer on disk", rather than leaving it out of the list entirely. A **Now card** sits directly ABOVE the progress spine — the session’s current line, in the main view rather than buried at the foot of the conversation, so what a session is doing right now is visible without scrolling; it hides itself both when there is no current line and when the session has gone stale. The conversation header’s **latest** control now scales with the icon-size setting like every other icon, in all three icon styles. Also carries a **progress spine** across a session's todos — which **zooms and scrolls**: the `All · 15m · 1h · 6h · 24h` chips set how much time fills the view, and anything tighter than `All` draws the strip wider than the panel so you scroll — or simply drag it — sideways through the session. Because it zooms rather than filters, nothing is ever hidden: a 2000-hour session stops crushing every event marker into the left edge without a single todo dropping out of the bar. The chips follow the Config → **Icon size** setting like every other glyph in the app, and a live treatment pulses the running segment and the NOW edge (all of it switched off under `prefers-reduced-motion`) — and — in the header's meta strip — the session's **model**; a background agent listed in the Agents panel carries its own model too, which can (and does) differ from its parent session's. The header's **PINNED** chip is a real pin/unpin toggle now — always shown, reading "Pin" unpinned and "Pinned" pinned — posting to the same `POST /api/pin` route the classic sidebar and rail already use, so pin state agrees across all three views; keyboard-operable, and not gated to localhost. The **N AGENTS RUNNING** pill beside it is clickable too, expanding and scrolling down to the Agents & shells panel, which keeps updating live on the same poll. The header also shows, when the session has any, its **open-flag count**, plus a permanent **stat chips** row (files · commands · reads · commits · tests · tokens · branch) — always visible on desktop, with no Config toggle any more, and hidden below the 600px phone breakpoint since the phone layout omits it. The Evidence column's **Run a command** panel now pops its output into a live pane — running / finished-exit-N / failed / empty states — over the same `/api/term/run` + `/api/term/stream` route the classic Command runner already uses; no new endpoint. Its **Terminal controls** panel — the model/effort switchers and context readout described under [Terminal](#terminal--on-by-default) — only renders when a live Claude CLI is actually the foreground process of an open terminal for that session, reusing the same check `/api/term/attached` already answers rather than a second implementation, and its model/effort buttons are now genuinely wired: clicking one posts `/api/term/inject` against the attached terminal (`term_tty`, a new field on the detail dict), the same route the classic dashboard's own switchers use. A phone-layout **stop** control posts `/api/term/close` the same way. Both stay honestly disabled, with copy explaining why, whenever no terminal is attached.
- **Theme** follows `prefers-color-scheme` live — a three-way **Auto / Light / Dark** control, with a persisted manual override (`tracker.theme` = `auto` | `light` | `dark`) — independent of the classic dashboard's own theme toggle. Icons inherit the active theme's colour automatically via CSS colour tokens. Dialogs opened from here — Config, Help, and the Narration/message overlay borrowed from the classic dashboard — now follow this same theme in both light and dark, rather than the classic dashboard's own palette; the classic dashboard's own dialogs are unchanged.
- **Terminal** — the same PTY plumbing as the classic dashboard in new chrome (`ExtVT.mountInto()` hosts it inline rather than a second terminal implementation). xterm.js still loads lazily, only once a terminal is actually opened. The overlay traps focus while open and restores it to whatever had focus before opening, the same as every other Control Room dialog. Its header's Config/Help/Close controls all carry visible text labels next to their icons now, Close included — previously the one icon-only holdout in that row.
- **Help and Config are dialogs, not routes.** Config's own preferences — theme, session rail (Auto / Open / Collapsed), cards-start-folded, desktop notifications + sound, **board tiles** (a 3–12 slider, default 8, client-side only — the board repaints the moment you move it), poll interval — are live browser settings that save as you change them. Below them, the real running parameters — **LIVE_WINDOW, terminal renderer, max terminals, terminal on/off, terminal app, and the command allowlist** — are now genuinely editable from the dialog (`GET`/`POST /api/config`), not just displayed: they persist to a gitignored `config.json`, read live (no restart needed), with precedence `config.json` > environment variable > built-in default. **PORT** and **HOST** are read-only display fields in the dialog, each labelled "takes effect on restart" — rebinding a live listening socket isn't attempted, so the row just reflects what the server is currently running with; the **terminal on/off** setting carries the same restart note. The **Interface section** includes **icon style** (`ICON_STYLE`: `icons` / `emoji` / `text`; default `icons`) and **icon size** (`ICON_SCALE`: 75–200%, default 100, with live preview as you adjust the slider). **`TRACKER_AUTH` is deliberately excluded from all of this** and still shows only as set/not-set — writing a password typed into a web form into a plaintext file on disk would be a real security regression, not a convenience.
...
| Progress spine (Control Room's todo bar) | ✅ time-proportional — an exact `started_at`/`ended_at` task-store id join | 🟡 time-proportional too, labelled *inferred time* — an approximate name-match join (chat-side task ids matched by normalised name), not an exact id join | ➖ equal-width fallback — no timing source exists to join in the first place |
...
| Degraded-transcript notice ("Couldn't read this session — a line failed to parse, everything before it is shown") | ✅ reports the failing line **number** (1-based JSONL line) | 🟡 reports the same notice, but with **no line number** — Auggie's task-storage is a family of separate files, not one file with lines. A session file that's corrupt *outright* (fails to parse at all) now degrades the same honest way when you open it directly — "couldn't read this session," not missing; it still won't turn up in the sidebar **list** itself, which silently skips a file it can't parse | ➖ n/a |
...
tests/test_icons.py           icon sprite integrity — unique symbol ids, well-formed SVG, no dangling
                               icon references, and no emoji in live code
