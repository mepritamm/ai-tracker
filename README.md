...
- **Board** — the home view: the session rail down the side, a triage strip, and a board of tiles that's never more than 8 at once (pinned sessions first, then by state and recency — everything past the cap lives in the rail, not on the board). Each tile shows the session's name and its working directory, a short "what it's doing now" line underneath, a done/total todo count, and — when the tool has recorded one — its **model** (e.g. `sonnet 4.5`), tacked onto the same trailing "age · tool" line. Rail rows carry the same directory + model detail, plus a **group-by** control: directory · activeness · last 24h · 7 days · 30 days · none.
...
- **Terminals** — the third top-bar destination; opens the same Manage-terminals panel the classic dashboard's **Manage** button does, badged with the live running count.
- **Session detail** — full-width, three columns of panels, every one collapsed by default except the conversation timeline, which merges narration and prompts into a single chat-style thread (newest entries at the top) instead of two separate panels. The TIMELINE panel header carries four individually-selectable filter chips — **prompts**, **narration**, **tools**, **results** — alongside the existing all/talk-only presets, plus a **pop-out** button to open the newest entry in the full-screen modal (or click any entry to open it); the same **openText()/_setNav()** modal the classic dashboard's narration pop-out uses. Adds a **Links** panel, a **progress spine** across a session's todos, and — in the header's meta strip — the session's **model**; a background agent listed in the Agents panel carries its own model too, which can (and does) differ from its parent session's. The header also shows **pinned** state and, when the session has any, its **open-flag count**.
- **Theme** follows `prefers-color-scheme` live — a three-way **Auto / Light / Dark** control, with a persisted manual override (`tracker.theme` = `auto` | `light` | `dark`) — independent of the classic dashboard's own theme toggle. Icons scale colour automatically with the active theme's palette using CSS colour tokens rather than filters.
...
- **Help and Config are dialogs, not routes.** Config's own preferences — theme, session rail (Auto / Open / Collapsed), cards-start-folded, poll interval, notification sound — are live browser settings that save as you change them. Below them, the real running parameters — **LIVE_WINDOW, terminal renderer, max terminals, terminal on/off, terminal app, and the command allowlist** — are now genuinely editable from the dialog (`GET`/`POST /api/config`), not just displayed: they persist to a gitignored `config.json`, read live (no restart needed), with precedence `config.json` > environment variable > built-in default. **PORT** and **HOST** are editable too, but only take effect on the next process start — the dialog says so on that row. **`TRACKER_AUTH` is deliberately excluded from all of this** and still shows only as set/not-set — writing a password typed into a web form into a plaintext file on disk would be a real security regression, not a convenience. Board tiles stay capped at 8 by design, so that row is read-only too.
...
- **Mobile / tablet.** The top bar wraps at narrow widths, every destination stays reachable, touch targets are 44px at phone width, and the rail becomes an overlay drawer below 1024px.
