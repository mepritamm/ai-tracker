...
- **Resuming a running background agent** — Claude Code refuses to `--resume` a session that's still running as a background agent, and `--fork-session` is the only programmatic way past it. Note what that gives you: a **copy**, opened under a new session id, with the original agent left running. There is no non-interactive way to attach to the live one — `claude agents` is an interactive picker with no attach verb and no id argument. See [`docs/claude-resume-command-matrix.md`](docs/claude-resume-command-matrix.md) for the tested behaviour of every session state.
