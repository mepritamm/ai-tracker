...
- **End-state at a glance** — a session **waiting on your answer** (an unanswered `AskUserQuestion` / Auggie `ask-user`) is flagged **⏳ answer** with an amber highlight so you know to go respond; one that **completed its last run** (idle, and the last turn was the assistant finishing) shows a subtle **✅ done**. Both work for every tool.
