...
**Main view** for the selected session — the header carries a progress ring, title, stat chips, and a **🚩 Flag an issue** toggle; below it the body splits into two halves, **◐ State** (progress, PRs, plan, decisions, summary) and **◑ Activity** (narration, prompts, files, commands), collapsing to one column on narrow screens. A **☀️/🌙 theme toggle** next to the 🔔 bell switches between the default dark and a warm-beige light mode (remembered across visits); background agents & shells live in a right-side drawer. The 🚩 Flags panel is opt-in — the header toggle reveals it above the State/Activity split.
