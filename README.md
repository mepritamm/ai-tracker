...
- **Model switcher and context readout** — a slim bar under the terminal shows the session's current context usage and its cumulative total, and (for Claude sessions) a model button that switches model mid-session. That button types `/model <name>` into the running CLI, because `/model` is a CLI slash command rather than anything the server can set — so it only appears on terminals actually running Claude. Context numbers come from the tool's **own** transcripts, not from scraping the screen; a percentage bar appears only when the tool records a context limit (Auggie does, Claude currently doesn't — so Claude sessions show the raw number rather than an invented percentage).
