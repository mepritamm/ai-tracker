...
Resolve the open issues the user raised with the 🚩 Flag button in the session tracker. Flags live in `flags.json` next to `tracker.py`; each is `{id, session, project, note, context, ts, resolved}`. The fix is almost always in `tracker.py` (the app) or in how it parses `~/.claude/projects/*/*.jsonl`.
...
2. **For each open flag, in order:**
   ...
   c. **Fix** in `tracker.py`. Match the existing style; keep it one stdlib file with no new dependencies unless the user opts in. Prefer the smallest change that holds.
   d. **Verify.** Run `python3 tracker.py --selfcheck`. Extend the self-check with one assertion covering the fixed behaviour. For UI/CSS changes, also boot the server (`PORT=88xx python3 tracker.py &`) and `curl` the relevant endpoint or page to confirm.
...
- **Live changes vs restart:** data files (`flags.json`, `titles.json`) are read per-request, so they refresh without a restart. Anything in `tracker.py` itself — including the served HTML/CSS in the `PAGE` string — needs the user to restart `python3 tracker.py`. Always tell the user which kind of change you made.
