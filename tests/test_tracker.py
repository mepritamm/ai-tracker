...
from aitracker import util as _util, store as _store, overview as _overview, registry as _registry
from aitracker.providers import claude as _claude, auggie as _auggie
from tests.test_selfcheck import _run as _selfcheck_fn


class _Shim:
    """Adapter so main's tracker.* test body runs against the aitracker package.
    Reads resolve across the modules; writes to the overridable path globals
    (PROJECTS/AUGMENT_DIR/…) route to config, which every module reads live."""
    _mods = (_config, _util, _store, _overview, _registry, _claude, _auggie)

    def __getattr__(self, k):
        if k == "_selfcheck":
            return _selfcheck_fn
        for m in self._mods:
            if hasattr(m, k):
                return getattr(m, k)
