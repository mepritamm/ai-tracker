...
ORDER = [
    ...
    "registry.py", "tunnel.py", "server.py", "cli.py",
]
...
import atexit, base64, datetime, difflib, errno, fcntl, glob, hashlib, hmac, json, os, re, secrets, shutil, signal, subprocess, sys, threading, time, traceback, urllib.parse, uuid, webbrowser
...
def _unified(*a, **kw):
    # providers/claude.py imports util.py's `unified` under the local alias `_unified` --
    # an intra-package alias, not a stdlib import, so the import-stripping above drops that
    # line entirely. util.py's own `unified` (flattened in below under its own name, ORDER
    # puts util.py right after config.py) is looked up at CALL time, not at def time, so this
    # indirection survives being defined here ahead of it.
    return unified(*a, **kw)
'''
...
def strip_module(src):
    ...
    # server.py does `from . import tunnel` and calls tunnel.status()/tunnel.cred()/etc --
    # once tunnel.py's own defs are flattened into this same single namespace (see ORDER
    # above) there is no `tunnel` object left to look those up on, same reasoning as config.
    body = re.sub(r'(?<![\w.])tunnel\.', '', body)   # single namespace — tunnel.X -> X
    return body.strip("\n")
